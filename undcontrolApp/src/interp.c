...
static long get_id_scan_gaps(aSubRecord *prec) {
  /* 
   * This routine does the 2d interpolation. The format of the waveform
   * wf[0] = number of elements in x 
   * wf[1] = x start
  ...
  double x_off   = ((double *)prec->c)[0];
  double y_off   = ((double *)prec->d)[0];
  double e_start = ((double *)prec->s)[0];
  double e_stop  = ((double *)prec->t)[0];
  ...
  //fprintf(stderr, "y_step = %f\n", y_step);
  //fprintf(stderr, "y_n = %ld\n", y_n);
  //fprintf(stderr. "e_start = %f\n", e_start);
  //fprintf(stderr, "e_stop = %f\n", e_stop);
  
  // If the energies are too close to eachother (zero), get out of here
  if (fabs(e_start-e_stop) < 1e-9 || e_start <= 0 || e_stop <= 0) {
    return 2;
  }

  long ie;
  for (ie = 0; ie < prec->nova; ++ie) {
    x_in = e_start + (e_stop - e_start) / (prec->nova - 1) * (double) ie;
    //fprintf(stderr, "x_in = %f\n", x_in);
    ...
    ((double *)(prec->vala))[ie] = r1 + ((r2 - r1) * y_r);
